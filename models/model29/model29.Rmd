...
- Same as Model 28 but only on people over 65

## 29 - Data Prep
```{r model29 modeling setup}
...
BE_vars.29 <- c('PopDens_two', 'RetDens_bg', 'distrail_walk_quarter', 'distrail_walk_one', 'NumBusStops_one', 'NetLoadDens_GF18_two', 'JobsWi45_transit_to_car_ratio', 
                #'NumBusStops_bg',
...
df.clean %>% select(!!!demo_controls.29)
```

## Pull dataset
```{r model29 pull dataset}
df.modeling29 <- 
  df.clean %>% 
  ...
  select(
    id,
    !!dv,
    !!!BE_vars.29, # Note: the !!!'s just de-reference the elements you created in the above chunk
    !!!demo_controls.29
  ) %>%
...
### 29 - Add Interaction terms
```{r model29 - add interaction terms}
...
df.modeling29[['loadDens_X_RetDens_above_med']] <- if_else(
    (df.modeling29$NetLoadDens_GF18_two > get_summary_val('NetLoadDens_GF18_two', 'median', df.scalars.29)) &
      (df.modeling29$RetDens_bg > get_summary_val('RetDens_bg', 'median', df.scalars.29)),
    1,0)

# loadDens_X_BusStops_below_thresh = if NetLoadDens Z < -.59 and NumBusStops_one < median
# NetLoadDens
Z.thresh.NetLoad <- -.5
thresh.NetLoad <- (Z.thresh.NetLoad * get_summary_val('NetLoadDens_GF18_two', 'sd', df.scalars.29)) +  get_summary_val('NetLoadDens_GF18_two', 'median', df.scalars.29)
thresh.NumBusStops <- get_summary_val('NumBusStops_one', 'mean', df.scalars.29)

df.modeling29[['loadDens_X_BusStops_below_thresh']] <- if_else(
  (df.modeling29$NetLoadDens_GF18_two < thresh.NetLoad) &
    (df.modeling29$NumBusStops_one < thresh.NumBusStops),
  1, 
  0
)

# add the new columns to the column list if they should be there
BE_vars.29 <- c(BE_vars.29, 'loadDens_X_RetDens_above_med', 'loadDens_X_BusStops_below_thresh')
```

## 29 - Scaling (pt. 2)
```{r model29 - scaling}
# Now Scale the BE-cols
BE_vars.29.to_scale <- c('PopDens_two', 'RetDens_bg', 'NetLoadDens_GF18_two', 'StreetDens_GF18_two','JobsWi45_transit_to_car_ratio', 'NumBusStops_one')

# sanity check
stopifnot(all(BE_vars.29.to_scale %in% BE_vars.29))

# scale vars
df.modeling29.scaled <- 
  df.modeling29 %>%
  mutate(across(
    !!BE_vars.29.to_scale,
...
#### 29 - Model Setup & Run
```{r model29 model setup}
library(fastDummies)
# dummify
df.modeling29.dummies <- fastDummies::dummy_cols(df.modeling29.scaled, remove_selected_columns = TRUE, remove_most_frequent_dummy = FALSE)

names.29 <- df.modeling29.dummies %>% names()
...
vars_to_remove.29 <- purrr::imap_chr(defaults_vars.29, \(x, idx) paste0(idx, '_', x)) 

# Set Vars
vars.29 <- names.29[3:length(names.29)]
vars.29 <- vars.29[vars.29 %notin% vars_to_remove.29]

form.29 <- as.formula(paste(dv, '~', paste0(vars.29, collapse='+')))

# Model
model.29 <- AER::tobit(form.29, left=0, data=df.modeling29.dummies)
...
which(is.na(coef(model.29))) # If you're fine, this will just be "named integer(0)"

# Save the raw output to an output file 
model.29 %>%
  tidy_tobit_model() %>%
  write_csv('output/model29_raw.csv')

model.29 %>%
  tidy_tobit_model() %>%
  write_csv('models/model29/model29_raw.csv')
```

### Set Variable Names
```{r model29 - set variable names}
new_names.29 <- 
  names(coef(model.29)) %>%
  ...
  I()
new_names.29 <- set_names(names(coef(model.29)), new_names.29)
new_names.29
...
#### 29 - Setup Plotting DF
```{r model29 generate plot df}
# Create Plotting DF
model.29.pltdf <- 
  model.29 %>%
  tidy_tobit_model() %>% 
  ...
  mutate(
    ...
    # Set Display Names
    Term = if_else(is.na(set_names(names(new_names.29), new_names.29)[v]), v, set_names(names(new_names.29), new_names.29)[v]), # <-- uses `new_names25` from previous chunk
...
model.29.pltdf$dist_band<- model.29.pltdf$Term %>%
...
# Join in VIF
vif.29 <- tibble(var = names(car::vif(model.29)), vif=car::vif(model.29))
...
plot_vif(model.29.pltdf, title = 'Model 29: VIF')

# Corrplot
### Similar - internal so its a simple plot
BE_vars.29.names <- set_names(
  model.29.pltdf[model.29.pltdf$grp == 'Built-Env.', ]$v,
  model.29.pltdf[model.29.pltdf$grp == 'Built-Env.', ]$Term 
)
plot_corrplot(df.modeling29.dummies, vars_named = BE_vars.29.names, title = 'Model 29: (65+) - CorrPlot')

# Coefs
model.29.pltdf %>% 
  plot_regression_coefs(remove_missing_flags = TRUE, title = 'Model 29 (65+) - Regression Coefficients (full)') + # for standard plots, not much to play around with
  theme( 
    # Increase font size
...
model.29.pltdf %>%
  ...
  plot_regression_coefs(
    title = 'Model 29 (65+) - Regression Coefficients (BE)', # for BE, you want to change some of the input flags
