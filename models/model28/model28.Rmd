...
- Same as Model 25 but messing with the threshold on Load Density and Bus Stops

## 28 - Data Prep
```{r model28 modeling setup}
...
BE_vars.28 <- c('PopDens_two', 'RetDens_bg', 'distrail_walk_quarter', 'distrail_walk_one', 'NumBusStops_one', 'NetLoadDens_GF18_two', 'JobsWi45_transit_to_car_ratio', 
                #'NumBusStops_bg',
...
df.clean %>% select(!!!demo_controls.28)
```

## Pull dataset
```{r model28 pull dataset}
df.modeling28 <- 
  df.clean %>% 
  ...
  select(
    id,
    !!dv,
    !!!BE_vars.28, # Note: the !!!'s just de-reference the elements you created in the above chunk
    !!!demo_controls.28
  ) %>%
...
### 28 - Add Interaction terms
```{r model28 - add interaction terms}
...
df.modeling28[['loadDens_X_RetDens_above_med']] <- if_else(
    (df.modeling28$NetLoadDens_GF18_two > get_summary_val('NetLoadDens_GF18_two', 'median', df.scalars.28)) &
      (df.modeling28$RetDens_bg > get_summary_val('RetDens_bg', 'median', df.scalars.28)),
    1,0)

# loadDens_X_BusStops_below_thresh = if NetLoadDens Z < -.59 and NumBusStops_one < median
# NetLoadDens
Z.thresh.NetLoad <- -.5
thresh.NetLoad <- (Z.thresh.NetLoad * get_summary_val('NetLoadDens_GF18_two', 'sd', df.scalars.28)) +  get_summary_val('NetLoadDens_GF18_two', 'median', df.scalars.28)
thresh.NumBusStops <- get_summary_val('NumBusStops_one', 'mean', df.scalars.28)

df.modeling28[['loadDens_X_BusStops_below_thresh']] <- if_else(
  (df.modeling28$NetLoadDens_GF18_two < thresh.NetLoad) &
    (df.modeling28$NumBusStops_one < thresh.NumBusStops),
  1, 
  0
)

# add the new columns to the column list if they should be there
BE_vars.28 <- c(BE_vars.28, 'loadDens_X_RetDens_above_med', 'loadDens_X_BusStops_below_thresh')
```

## 28 - Scaling (pt. 2)
```{r model28 - scaling}
# Now Scale the BE-cols
BE_vars.28.to_scale <- c('PopDens_two', 'RetDens_bg', 'NetLoadDens_GF18_two', 'StreetDens_GF18_two','JobsWi45_transit_to_car_ratio', 'NumBusStops_one')

# sanity check
stopifnot(all(BE_vars.28.to_scale %in% BE_vars.28))

# scale vars
df.modeling28.scaled <- 
  df.modeling28 %>%
  mutate(across(
    !!BE_vars.28.to_scale,
...
#### 28 - Model Setup & Run
```{r model28 model setup}
library(fastDummies)
# dummify
df.modeling28.dummies <- fastDummies::dummy_cols(df.modeling28.scaled, remove_selected_columns = TRUE, remove_most_frequent_dummy = FALSE)

names.28 <- df.modeling28.dummies %>% names()
...
vars_to_remove.28 <- purrr::imap_chr(defaults_vars.28, \(x, idx) paste0(idx, '_', x)) 

# Set Vars
vars.28 <- names.28[3:length(names.28)]
vars.28 <- vars.28[vars.28 %notin% vars_to_remove.28]

form.28 <- as.formula(paste(dv, '~', paste0(vars.28, collapse='+')))

# Model
model.28 <- AER::tobit(form.28, left=0, data=df.modeling28.dummies)
...
which(is.na(coef(model.28))) # If you're fine, this will just be "named integer(0)"

# Save the raw output to an output file 
model.28 %>%
  tidy_tobit_model() %>%
  write_csv('output/model28_raw.csv')

model.28 %>%
  tidy_tobit_model() %>%
  write_csv('models/model28/model28_raw.csv')
```

### Set Variable Names
```{r model28 - set variable names}
new_names.28 <- 
  names(coef(model.28)) %>%
  ...
  I()
new_names.28 <- set_names(names(coef(model.28)), new_names.28)
new_names.28
...
#### 28 - Setup Plotting DF
```{r model28 generate plot df}
# Create Plotting DF
model.28.pltdf <- 
  model.28 %>%
  tidy_tobit_model() %>% 
  ...
  mutate(
    ...
    # Set Display Names
    Term = if_else(is.na(set_names(names(new_names.28), new_names.28)[v]), v, set_names(names(new_names.28), new_names.28)[v]), # <-- uses `new_names25` from previous chunk
...
model.28.pltdf$dist_band<- model.28.pltdf$Term %>%
...
# Join in VIF
vif.28 <- tibble(var = names(car::vif(model.28)), vif=car::vif(model.28))
...
plot_vif(model.28.pltdf, title = 'Model 28: VIF')

# Corrplot
### Similar - internal so its a simple plot
BE_vars.28.names <- set_names(
  model.28.pltdf[model.28.pltdf$grp == 'Built-Env.', ]$v,
  model.28.pltdf[model.28.pltdf$grp == 'Built-Env.', ]$Term 
)
plot_corrplot(df.modeling28.dummies, vars_named = BE_vars.28.names, title = 'Model 28: CorrPlot')

# Coefs
model.28.pltdf %>% 
  plot_regression_coefs(remove_missing_flags = TRUE, title = 'Model 28 - Regression Coefficients (full)') + # for standard plots, not much to play around with
  theme( 
    # Increase font size
...
model.28.pltdf %>%
  ...
  plot_regression_coefs(
    title = 'Model 28 - Regression Coefficients (BE)', # for BE, you want to change some of the input flags
