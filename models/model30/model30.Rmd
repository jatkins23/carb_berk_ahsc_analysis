...
- Same as Model 28 but only on people over 65 and hh-income under $75k

## 30 - Data Prep
```{r model30 modeling setup}
...
BE_vars.30 <- c('PopDens_two', 'RetDens_bg', 'distrail_walk_quarter', 'distrail_walk_one', 'NumBusStops_one', 'NetLoadDens_GF18_two', 'JobsWi45_transit_to_car_ratio', 
                #'NumBusStops_bg',
...
df.clean %>% select(!!!demo_controls.30)
```

## Pull dataset
```{r model30 pull dataset}
df.modeling30 <- 
  df.clean %>% 
  ...
  filter(
    ...
    age_OE___65_and_up == 1,
    hhincome_OE____75k_and_up == 0 ### <- add this
  ) %>%
  ...
  select(
    id,
    !!dv,
    !!!BE_vars.30, # Note: the !!!'s just de-reference the elements you created in the above chunk
    !!!demo_controls.30
  ) %>%
  ...
  select(
    ...
    # add unnecessary age columns
    -c(age___Missing, age_OE___15_and_up, age_OE___25_and_up, age_OE___35_and_up, age_OE___45_and_up, age_OE___55_and_up, age_OE___65_and_up),
    # add unnecessary hh-income columns
    -c(hhincome___Missing, hhincome_OE____75k_and_up, hhincome_OE___100k_and_up, , hhincome_OE___125k_and_up, hhincome_OE___150k_and_up, hhincome_OE___200k_and_up)
  )
...
### 30 - Add Interaction terms
```{r model30 - add interaction terms}
...
df.modeling30[['loadDens_X_RetDens_above_med']] <- if_else(
    (df.modeling30$NetLoadDens_GF18_two > get_summary_val('NetLoadDens_GF18_two', 'median', df.scalars.30)) &
      (df.modeling30$RetDens_bg > get_summary_val('RetDens_bg', 'median', df.scalars.30)),
    1,0)

# loadDens_X_BusStops_below_thresh = if NetLoadDens Z < -.59 and NumBusStops_one < median
# NetLoadDens
Z.thresh.NetLoad <- -.5
thresh.NetLoad <- (Z.thresh.NetLoad * get_summary_val('NetLoadDens_GF18_two', 'sd', df.scalars.30)) +  get_summary_val('NetLoadDens_GF18_two', 'median', df.scalars.30)
thresh.NumBusStops <- get_summary_val('NumBusStops_one', 'mean', df.scalars.30)

df.modeling30[['loadDens_X_BusStops_below_thresh']] <- if_else(
  (df.modeling30$NetLoadDens_GF18_two < thresh.NetLoad) &
    (df.modeling30$NumBusStops_one < thresh.NumBusStops),
  1, 
  0
)

# add the new columns to the column list if they should be there
BE_vars.30 <- c(BE_vars.30, 'loadDens_X_RetDens_above_med', 'loadDens_X_BusStops_below_thresh')
```

## 30 - Scaling (pt. 2)
```{r model30 - scaling}
# Now Scale the BE-cols
BE_vars.30.to_scale <- c('PopDens_two', 'RetDens_bg', 'NetLoadDens_GF18_two', 'StreetDens_GF18_two','JobsWi45_transit_to_car_ratio', 'NumBusStops_one')

# sanity check
stopifnot(all(BE_vars.30.to_scale %in% BE_vars.30))

# scale vars
df.modeling30.scaled <- 
  df.modeling30 %>%
  mutate(across(
    !!BE_vars.30.to_scale,
...
#### 30 - Model Setup & Run
```{r model30 model setup}
library(fastDummies)
# dummify
df.modeling30.dummies <- fastDummies::dummy_cols(df.modeling30.scaled, remove_selected_columns = TRUE, remove_most_frequent_dummy = FALSE)

names.30 <- df.modeling30.dummies %>% names()
...
vars_to_remove.30 <- purrr::imap_chr(defaults_vars.30, \(x, idx) paste0(idx, '_', x)) 

# Set Vars
vars.30 <- names.30[3:length(names.30)]
vars.30 <- vars.30[vars.30 %notin% vars_to_remove.30]

form.30 <- as.formula(paste(dv, '~', paste0(vars.30, collapse='+')))

# Model
model.30 <- AER::tobit(form.30, left=0, data=df.modeling30.dummies)
...
which(is.na(coef(model.30))) # If you're fine, this will just be "named integer(0)"

# Save the raw output to an output file 
model.30 %>%
  tidy_tobit_model() %>%
  write_csv('output/model30_raw.csv')

model.30 %>%
  tidy_tobit_model() %>%
  write_csv('models/model30/model30_raw.csv')
```

### Set Variable Names
```{r model30 - set variable names}
new_names.30 <- 
  names(coef(model.30)) %>%
  ...
  I()
new_names.30 <- set_names(names(coef(model.30)), new_names.30)
new_names.30
...
#### 30 - Setup Plotting DF
```{r model30 generate plot df}
# Create Plotting DF
model.30.pltdf <- 
  model.30 %>%
  tidy_tobit_model() %>% 
  ...
  mutate(
    ...
    # Set Display Names
    Term = if_else(is.na(set_names(names(new_names.30), new_names.30)[v]), v, set_names(names(new_names.30), new_names.30)[v]), # <-- uses `new_names25` from previous chunk
...
model.30.pltdf$dist_band<- model.30.pltdf$Term %>%
...
# Join in VIF
vif.30 <- tibble(var = names(car::vif(model.30)), vif=car::vif(model.30))
...
plot_vif(model.30.pltdf, title = 'Model 30: VIF')

# Corrplot
### Similar - internal so its a simple plot
BE_vars.30.names <- set_names(
  model.30.pltdf[model.30.pltdf$grp == 'Built-Env.', ]$v,
  model.30.pltdf[model.30.pltdf$grp == 'Built-Env.', ]$Term 
)
plot_corrplot(df.modeling30.dummies, vars_named = BE_vars.30.names, title = 'Model 30: (65+, < $75k) - CorrPlot')

# Coefs
model.30.pltdf %>% 
  plot_regression_coefs(remove_missing_flags = TRUE, title = 'Model 30 (65+, < $75k) - Regression Coefficients (full)') + # for standard plots, not much to play around with
  theme( 
    # Increase font size
...
model.30.pltdf %>%
  ...
  plot_regression_coefs(
    title = 'Model 30 (65+, < $75k) - Regression Coefficients (BE)', # for BE, you want to change some of the input flags
